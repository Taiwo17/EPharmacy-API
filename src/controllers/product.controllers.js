...
const { deleteFile, uploadBuffer } = require('../services/storage.services')
...
const create = asyncHandler(async (req, res) => {
  ...
  try {
    // Upload every image received from Multer
    for (const file of req.files || []) {
      const uploadedFile = await uploadBuffer(file.buffer, {
    ...
    // Create the product after image uploads succeed
    const product = await Product.create({
      ...req.body,
      sku,
      images: uploadedImages,
      requiresRx: req.body.requiresRx === 'true',
    ...
    for (const image of uploadedImages) {
      try {
        ...
        console.error('Failed to delete uploaded image:', deleteError.message)
      }
    }
